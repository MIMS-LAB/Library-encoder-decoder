...
uint8_t encode(double data, uint8_t header, uint32_t time, uint8_t *out)
{
    ...
    for(uint8_t i = 0; i < RRC_DATAPACK_SIZE; i++)
        out[i] = (header & RRC_MASK_HEADER) << RRC_SHIFT_HEADER;
    
    out[RRC_DATAPACK_SIZE - 1] = (RRC_HEAD_END & RRC_MASK_HEADER) << RRC_SHIFT_HEADER;

    //  store the checksum in the first packet
    out[0] |= (checks & RRC_MASK_CHECKS) << RRC_SHIFT_CHECKS;
