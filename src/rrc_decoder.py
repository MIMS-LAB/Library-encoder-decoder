...
def readByte():
	...
	if inSerialBuffer.inWaiting() == 0:
		return None

	return int.from_bytes(inSerialBuffer.read(), byteorder="big", signed=False)



def getPackets(retries=-1):
	'''
	getPackets() --> list
	...
	while retries > 0 or retries < 0:
		...
		for i in range(6):                            #  read 6 more bytes
			...
			packet.append(byte) #  read one byte from buffer
		...
		packet.append(byte) #  read the last byte
...
def generateChecksum(data):
	...
	'''
	result  = 0x01 & data & 0x01                     #  LSB of lowest byte
	result |= 0x02 & ((data & 0x80)    >> 6)         #  MSB of lowest byte
	result |= 0x04 & ((data & 0x8000)  >> 13)        #  MSB of middle byte
	result |= 0x08 & ((data & 0x10000) >> 13)        #  LSB of highes byte
